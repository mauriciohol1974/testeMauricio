...
public class HisCedManterIniciar extends HistoricoCedenteEstrategia {
    ...
    public String processRequest(HttpServletRequest request,
        ...
        // Recupera os beans
        HistoricoCedenteBean historicoCedenteBean = new HistoricoCedenteBean();
        CedenteCabecaBean cedCabBean = new CedenteCabecaBean();
        cedCabBean = (CedenteCabecaBean) cedCabBean.newBean();
        PilhaVoltarControle.init(request);

        if (fluxo.equals(FLUXO_NORMAL)) {
            historicoCedenteBean = (HistoricoCedenteBean) historicoCedenteBean.newBean();
        } else {
            historicoCedenteBean = (HistoricoCedenteBean) historicoCedenteBean.getSessionBean(request,
                    FILTRO_BEAN);
        }

        // para gerar informacoes para tela de erro
        request.getSession().setAttribute(FILTRO_BEAN, historicoCedenteBean);
        request.getSession().setAttribute(DATA_BEAN, historicoCedenteBean);
