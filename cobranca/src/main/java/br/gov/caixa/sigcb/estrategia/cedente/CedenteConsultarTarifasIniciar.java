 ...
 */
public class CedenteConsultarTarifasIniciar extends CedenteConsultarEstrategia {
    ...
    public String processRequest(HttpServletRequest request,
        ...
        BeanList blDefault = handler.executeSimpleTransactionQuery(tarifasBean,
        		transUser);

        ArrayList alTarifas = new ArrayList();

        // Se alteracao da guia, carregar campos
        if (CedenteEstrategia.ALTERACAO_EM_INCLUSAO.equals(tarifasBean.getSituacaoGuia())) {
            tarifasBean.setTipoConsulta("I");
            transUser = TRANSACAO_CONSULTAR_INFORME_TARIFAS + usuarioBean.getCodigoUsuario().toUpperCase();
            BeanList blConsulta = handler.executeSimpleTransactionQuery(tarifasBean,
            		transUser);
            alTarifas = this.montaArrayList(blDefault, blConsulta);
        } else {
            alTarifas = this.convertDataStructure(blDefault.iterator());
        }
        ...
        return PAGE_CONSULTAR_INFORME_TARIFAS;
    }
