<%/***********************************************
             ...
             *Projeto CAIXA - SIGCB
             *Componente: soagtit_incluir.jsp - Java Server Pages
...
<%SolicitacaoAgendamentoBean solicitacaoBean = (session.getAttribute(SolicitacaoAgendamentoEstrategia.FILTRO_BEAN) == null
                    ? new SolicitacaoAgendamentoBean()
                    : (SolicitacaoAgendamentoBean) session.getAttribute(SolicitacaoAgendamentoEstrategia.FILTRO_BEAN));
            solicitacaoBean.setCodigoCedente(session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_ATUAL) == null
                    ? new Long(0)
                    : (Long) session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_ATUAL));
            CedenteCabecaBean cedCabBean = (session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_CABECALHO_BEAN) == null
                    ? new CedenteCabecaBean()
                    : (CedenteCabecaBean) session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_CABECALHO_BEAN));
            CedenteGeralBean cedGeralBean = (session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_GERAL_BEAN) == null
                    ? new CedenteGeralBean()
                    : (CedenteGeralBean) session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_GERAL_BEAN));
            CedenteBloquetosBean cedBloqBean = (session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_BLOQUETO_BEAN) == null
                    ? new CedenteBloquetosBean()
                    : (CedenteBloquetosBean) session.getAttribute(SolicitacaoAgendamentoEstrategia.CEDENTE_BLOQUETO_BEAN));
            

            if ( !cedGeralBean.getPercentualJurosDia().toValorSemPonto().equalsIgnoreCase("000")) {
                 	solicitacaoBean.setTipoJuroDia(new Long(3));
                 	solicitacaoBean.setPrazoJuros(new Long(1));
            }else{
                	solicitacaoBean.setTipoJuroDia(new Long(5));
                  	solicitacaoBean.setPrazoJuros(new Long(0)); 
            }
...
<p:Document>
	<s:FormStrategy name="frmMain" action="SigcbControle"
		estrategia="<%=SolicitacaoAgendamentoEstrategia.STRATEGY_INCLUIR_FINALIZAR%>"
		fluxo="normal">
		<s:Menu />
		<s:Titulo
			name="<%=SolicitacaoAgendamentoEstrategia.PAGE_TITLE_INCLUIR%>" />
		<input type="hidden" name="moeda">
		<input type="hidden" name="codigoCedente"
			value='<%=solicitacaoBean.getCodigoCedente()%>'>
		<input type="hidden" name="codigoBancoSacado"
			value='<%=solicitacaoBean.getCodigoBancoSacado()%>'>
		<input type="hidden" name="registro">
		<input type="hidden" name="indicadorSolicitacao"
			value='<%=solicitacaoBean.getIndicadorSolicitacao()%>'>
		<input type="hidden" name="nomeBancoSacado"
			value='<%=solicitacaoBean.getNomeBancoSacado()%>'>
		...
		<%if (solicitacaoBean.getIndicadorSolicitacao()
                            .equals(new Long(7))) {%>
		...
		<table width="777" BORDER="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<table width="95%" border="0" cellspacing="1" cellpadding="0"
					...
					<tr valign="top">
						<td colspan="4" class="textoTitulo"><%=solicitacaoBean.getIndicadorSolicitacaoTextoInfinitivo()%>
						Emissão de Títulos:
						<hr>
						</td>
					</tr>
					...
					<tr>
						...
						<td width="26%"><%//EAM 14/10/2005 - Habilitado o campo para permitir sem registro.%>
						<s:ComboSimNao name="registroCombo"
							onChange="javascript:configuraCampoDependente(this)"
							selectedValue='<%=solicitacaoBean.getRegistro().trim().equals("")?"S":solicitacaoBean.getRegistro()%>' />
						</td>
						<td class="textoTitulo" width="17%">Espécie:</td>
						<td width="26%"><s:ComboEspecieTitulo name="especie"
							selectedValue='<%=solicitacaoBean.getEspecie().equals(new Long(0))?"-1":solicitacaoBean.getEspecie().toString()%>'
							branco="true" /></td>
					</tr>

					<tr>
						<td class="textoTitulo" width="17%">Forma Obtenção Valor Título:</td>
						<td width="26%"><s:ComboFormaObtencao name="formaObtencaoValor" 
							selectedValue='<%=solicitacaoBean.getFormaObtencaoValor().equals(new Long(0))?"1":solicitacaoBean.getFormaObtencaoValor().toString()%>' 							
							onChange="configuraCampoDependenteFOV(this)" 
							/> </td> <%//AL - 07/01/2009 - SISOL 250475 - Após habilitar o combo relacionado ao campo formaObtencaoValor, retirar o campo hidden%>
						<td class="textoTitulo" width="17%">Valor:</td>
						<td width="26%"><p:InputCurrency name="valorSolicitacao"
							maxlength="15" size="27" CLASS="inputtext"
							...
							onBlur="configuraCampoDependente(this)" /></td>
					</tr>
					

					<tr>
						<td class="textoTitulo" width="17%" nowrap>Forma Obtenção Envio
						Boleto:</td>
						<td width="26%"><s:ComboFormaObtencao
							name="formaObtencaoBloquetoCombo" 
							selectedValue='<%=solicitacaoBean.getFormaObtencaoBloqueto().equals(new Long(0))?"1":solicitacaoBean.getFormaObtencaoBloqueto().toString()%>'
							onChange="configuraCampoDependente(this)" /></td>
						<td class="textoTitulo" width="17%">Envio do Boleto:</td>
						<td width="26%"><s:ComboEnvioBloqueto name="envioBloqueto"
							onChange="ConfiguraSMS()" 
							selectedValue='<%=solicitacaoBean.getEnvioBloqueto().equals(new Long(0))?cedBloqBean.getEnvioBloqueto().toString():solicitacaoBean.getEnvioBloqueto().toString()%>'
							branco="true" /></td>
					</tr>
			    		...
			    		<td class="textoTitulo" width="17%" nowrap>&nbsp;</td>
              			...
              			<td width="26%">
              			 <select size="1" name="tipoSMS" disabled="disabled">
							...
							<option value="01">Informativa</option>
							<option value="02">Repr. Numérica</option>
							<option value="03">PEC</option>
						</select>   
                    	</td>	
			 		</tr>
					...
					<tr>
						...
						<td width="26%"><s:ComboSimNao name="endossoCombo"
							selectedValue='<%=solicitacaoBean.getEndosso().trim().equals("")?"N":solicitacaoBean.getEndosso()%>'
							disabled='true' /></td>
						<td class="textoTitulo" width="17%">Aceite:</td>
						<td width="26%"><s:ComboAceite name="aceiteCombo"
							selectedValue='<%=solicitacaoBean.getAceite().trim().equals("")?"-1":solicitacaoBean.getAceite()%>'
							branco="true" /></td>
					</tr>
					<tr>
						<td class="textoTitulo" width="17%">Protesto Automático:</td>
						<td width="26%"><s:ComboSimNao name="protestoAutomaticoCombo"
							disabled="true" selectedValue='N' /> <%//EAM 14/10/2005 - =solicitacaoBean.getProtestoAutomatico().trim().equals("")?cedGeralBean.getProtestoAutomatico():solicitacaoBean.getProtestoAutomatico()%>
						</td>
                    ...
                    //EAM 14/10/2005 prazo = cedGeralBean.getPrazoProtesto().toString();
                    //}else if (cedGeralBean.getProtestoAutomatico().trim().equals("N")){
                    prazo = cedGeralBean.getPrazoDevolucao().toString();
                    //}

                    %>
						<td class="textoTitulo" width="17%">Prazo Protesto/Devolução:</td>
						<td width="26%"><p:InputNumerico name="prazoProtestoDevolucao"
							maxlength="3" size="3" CLASS="inputtext"
							value='<%=solicitacaoBean.getPrazoProtestoDevolucao().equals(new Long(0))?prazo:solicitacaoBean.getPrazoProtestoDevolucao().toString()%>'
							onFocus="javascript: prevFocus(this);"
					...
					<tr>
						...
						<td class="textoValor" width="26%"><s:ComboMoeda name="moedaCombo"
							selectedValue='<%=solicitacaoBean.getMoeda().equals(new Long(0))?"9":solicitacaoBean.getMoeda().toString()%>'
							disabled="true" /></td>
						<%if (solicitacaoBean.getIndicadorSolicitacao()
                            .equals(new Long(6))) {%>
						...
						<td class="textoValor" width="26%"><p:InputDate
							name="dataVencimento"
							value='<%=solicitacaoBean.getDataVencimentoFormatada().equals("01/01/0001")?"":solicitacaoBean.getDataVencimentoFormatada()%>'
					...
					<tr>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tr class="headerLista">
								...
								<td nowrap width="18%" align="left"><%if (solicitacaoBean.getIndicadorSolicitacao()
                            .equals(new Long(6))) {%>
								Data <%}%></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td class="textoValor" width="8%" align="left"><p:InputPercentual
									name="percentualDesconto1" maxlength="5" size="9"
									CLASS="inputtext"
									...
									onBlur="javascript: configuraCampoDependenteFOV(this);" /></td>
								<td class="textoValor" width="24%" align="left"><p:InputCurrency
									name="valorDesconto1" maxlength="15" size="27"
									CLASS="inputtext"
									value='<%=solicitacaoBean.getValorDesconto1().toString().equals("R$ 0,00")?"":solicitacaoBean.getValorDesconto1().toString()%>'
									onFocus="javascript: prevFocus(this);configuraCampoDependenteFOV(this);"
									onKeyUp="javascript: focoPrazo(event.keyCode, this, document.frmMain.prazoDesconto1, document.frmMain.dataDesconto1);"
									onBlur="javascript: configuraCampoDependenteFOV(this);" /></td>
								<td class="textoTitulo" width="17%" align="left">Prazo Limite 1:
								</td>
								<td class="textoValor" width="8%" align="left"><%//EAM - 02/05/05 - SISOT 69(Massa do gestor)
                    //Correção para não perder as datas de desconto quando o fluxo = voltar
                    String prazo1 = "", prazo2 = "", prazo3 = "";
                    if (!solicitacaoBean.getPercentualDesconto1()
                            .toString()
                            .equals("0,00 %")
                        || !solicitacaoBean.getValorDesconto1()
                                .toString()
                                .equals("R$ 0,00")) {
                        if (!solicitacaoBean.getPrazoDesconto1()
                                .equals(new Long(0))
                            && solicitacaoBean.getDataDesconto1Formatada()
                                    .equals("")) {
                            prazo1 = solicitacaoBean.getPrazoDesconto1()
                                    .toString();
                        } else if (solicitacaoBean.getPrazoDesconto1()
                                .equals(new Long(0))
                                   && solicitacaoBean.getDataDesconto1Formatada()
                                           .equals("")) {
                            prazo1 = solicitacaoBean.getPrazoDesconto1()
                    ...
                    if (!solicitacaoBean.getPercentualDesconto2()
                            .toString()
                            .equals("0,00 %")
                        || !solicitacaoBean.getValorDesconto2()
                                .toString()
                                .equals("R$ 0,00")) {
                        if (!solicitacaoBean.getPrazoDesconto2()
                                .equals(new Long(0))
                            && solicitacaoBean.getDataDesconto2Formatada()
                                    .equals("")) {
                            prazo2 = solicitacaoBean.getPrazoDesconto2()
                                    .toString();
                        } else if (solicitacaoBean.getPrazoDesconto2()
                                .equals(new Long(0))
                                   && solicitacaoBean.getDataDesconto2Formatada()
                                           .equals("")) {
                            prazo2 = solicitacaoBean.getPrazoDesconto2()
                    ...
                    if (!solicitacaoBean.getPercentualDesconto3()
                            .toString()
                            .equals("0,00 %")
                        || !solicitacaoBean.getValorDesconto3()
                                .toString()
                                .equals("R$ 0,00")) {
                        if (!solicitacaoBean.getPrazoDesconto3()
                                .equals(new Long(0))
                            && solicitacaoBean.getDataDesconto3Formatada()
                                    .equals("")) {
                            prazo3 = solicitacaoBean.getPrazoDesconto3()
                                    .toString();
                        } else if (solicitacaoBean.getPrazoDesconto3()
                                .equals(new Long(0))
                                   && solicitacaoBean.getDataDesconto3Formatada()
                                           .equals("")) {
                            prazo3 = solicitacaoBean.getPrazoDesconto3()
                    ...
                    %> <p:InputNumerico name="prazoDesconto1"
									maxlength="3" size="3" CLASS="inputtext" value='<%= prazo1%>'
									onFocus="javascript: prevFocus(this);configuraCampoDependenteFOV(this);"
									onKeyUp="javascript: focoValor(event.keyCode, this, document.frmMain.percentualDesconto2, document.frmMain.valorDesconto2);"
									onBlur="javascript: configuraCampoDependenteFOV(this);" /></td>

								<td class="textoValor" width="18%" align="left"><%if (solicitacaoBean.getIndicadorSolicitacao()
                            .equals(new Long(6))) {%>
								<p:InputDate name="dataDesconto1" CLASS="inputtext"
							...
							<tr>
								...
								<td class="textoValor" width="8%" align="left"><p:InputPercentual
									name="percentualDesconto2" maxlength="5" size="9"
									CLASS="inputtext"
								...
								<td class="textoValor" width="18%" align="left"><%if (solicitacaoBean.getIndicadorSolicitacao()
                            .equals(new Long(6))) {%>
								<p:InputDate name="dataDesconto2" CLASS="inputtext"
							...
							<tr>
								...
								<td class="textoValor" width="8%" align="left"><p:InputPercentual
									name="percentualDesconto3" maxlength="5" size="9"
									CLASS="inputtext"
									...
									onBlur="javascript: configuraCampoDependenteFOV(this);" /></td>
								<td class="textoValor" width="24%" align="left"><p:InputCurrency
									name="valorDesconto3" maxlength="15" size="27"
									CLASS="inputtext"
									value='<%=solicitacaoBean.getValorDesconto3().toString().equals("R$ 0,00")?"":solicitacaoBean.getValorDesconto3().toString()%>'
									onFocus="javascript: prevFocus(this);configuraCampoDependenteFOV(this);"
									onKeyUp="javascript: focoPrazo(event.keyCode, this, document.frmMain.prazoDesconto3, document.frmMain.dataDesconto3);"
									onBlur="javascript: configuraCampoDependenteFOV(this);" /></td>
								<td class="textoTitulo" width="17%" align="left">Prazo Limite 3:
								</td>
								<td class="textoValor" width="8%" align="left"><p:InputNumerico
									name="prazoDesconto3" maxlength="3" size="3" CLASS="inputtext"
									value='<%=prazo3%>'
									onFocus="javascript: prevFocus(this);configuraCampoDependenteFOV(this);"
								...
								<td class="textoValor" width="18%" align="left"><%if (solicitacaoBean.getIndicadorSolicitacao()
                            .equals(new Long(6))) {%>
								<p:InputDate name="dataDesconto3" CLASS="inputtext"
							...
							<tr>
								...
								<td class="textoValor" width="8%" align="left"><p:InputPercentual
									name="percentualMulta" maxlength="5" size="9" CLASS="inputtext"
									value='<%=solicitacaoBean.getPercentualMulta().toString().equals("0,00 %")?cedGeralBean.getMulta().toString():solicitacaoBean.getPercentualMulta().toString()%>'
									onFocus="javascript: prevFocus(this);configuraCampoDependenteFOV(this);"
									onKeyUp="javascript: foco(event.keyCode, this, document.frmMain.prazoMulta);"
									onBlur="javascript: configuraCampoDependenteFOV(this);" /></td>
								<td class="textoValor" width="24%" align="left"><p:InputCurrency
									name="valorMulta" maxlength="15" size="27" CLASS="inputtext"
								...
								Multa:</td>
								<td class="textoValor" width="8%" align="left"><p:InputNumerico
									name="prazoMulta" maxlength="3" size="3" CLASS="inputtext"
									value='<%=solicitacaoBean.getPrazoMulta().equals(new Long(0))?cedGeralBean.getPrazoMulta().toString():solicitacaoBean.getPrazoMulta().toString()%>'
									onFocus="javascript: prevFocus(this);configuraCampoDependenteFOV(this);"
								...
								<td class="textoValor" width="18%" align="left"><%if (solicitacaoBean.getIndicadorSolicitacao()
                            .equals(new Long(6))) {%>
								<p:InputDate name="dataMulta" CLASS="inputtext"
						...
						<tr>
									...
									<td class="textoValor" width="26%"><p:InputPercentual
									name="percentualJuroMes" maxlength="5" size="9" CLASS="inputtext"
									value='<%=solicitacaoBean.getPercentualJuroMes().toString().equals("0,00 %")?cedGeralBean.getPercentualJurosDia().toString():solicitacaoBean.getPercentualJuroMes().toString()%>'
									onFocus="javascript: prevFocus(this);"
									...
									<td class="textoValor" width="24%" align="left"><p:InputCurrency
									name="valorJuros" maxlength="15" size="27" CLASS="inputtext"
									value='<%=solicitacaoBean.getValorJuros().toString().equals("R$ 0,00")?"":solicitacaoBean.getValorJuros().toString()%>'
									onFocus="javascript: prevFocus(this);configuraCampoDependenteFOV(this);"
									onKeyUp="javascript: foco(event.keyCode, this, document.frmMain.prazoMulta);"
									onBlur="javascript: configuraCampoDependenteFOV(this);" /></td>
									...
									<%if (solicitacaoBean.getIndicadorSolicitacao().equals(new Long(7))) {%>
									<p:InputNumerico
									name="prazoJuros" maxlength="3" size="3" CLASS="inputtext"
									value='<%=solicitacaoBean.getPrazoJuros().equals(new Long(0))?"0":solicitacaoBean.getPrazoJuros().toString()%>'/></td>
									<%} %>
									<td class="textoValor" width="18%" align="left">
									<%if (!solicitacaoBean.getIndicadorSolicitacao().equals(new Long(7))) {%>
									<p:InputDate name="dataJuros" CLASS="inputtext"
									value='<%=solicitacaoBean.getDataJurosFormatada().equals("")?"":solicitacaoBean.getDataJurosFormatada().toString()%>'
									onFocus="javascript: prevFocus(this);" /> 
									<%} %>
								</td>
						</tr>
						...
						<table width="95%" border="0" cellspacing="0" cellpadding="0"
							...
							<tr>
								...
								<td class="textoValor" width="26%"><p:InputNumerico
									name="diaVencimento" maxlength="2" size="2" CLASS="inputtext"
								...
								<td class="textoValor" width="26%"><s:ComboFormaVencimento
									name="formaVencimento"
									selectedValue='<%=solicitacaoBean.getFormaVencimento().equals(new Long(0))?"1":solicitacaoBean.getFormaVencimento().toString()%>' />
								</td>
							</tr>
							...
							<tr>
								...
								<td class="textoValor" width="26%"><p:InputNumerico
									name="diaEmissao" maxlength="2" size="2" CLASS="inputtext"
								...
								<td class="textoValor" width="26%"><s:ComboFormaEmissaoBloqueto
									name="formaEmissao"
									selectedValue='<%=solicitacaoBean.getFormaEmissao().equals(new Long(0))?"1":solicitacaoBean.getFormaEmissao().toString()%>' />
								</td>
							</tr>
							...
							<tr>
								...
								<td class="textoValor" width="26%"><s:ComboSimNao
									name="agendamentoPermanente"
									selectedValue='<%=solicitacaoBean.getAgendamentoPermanente().trim().equals("")?"N":solicitacaoBean.getAgendamentoPermanente()%>' />
								</td>
								<td class="textoTitulo" width="17%">&nbsp;</td>
								<td class="textoValor" width="26%">&nbsp;</td>
							</tr>
	    ...
	    function formSubmit() {
        if (validaSubmit()) {
	    		if (confirm(<%=solicitacaoBean.getIndicadorSolicitacao().equals(new Long(6))?"conf('143','Solicitação de Emissão de Títulos')":"conf('100','Agendamento de Emissão de Títulos')"%>)){
	    ...
	    function formSubmit_Voltar() {
    		if (confirm(conf("103"))) {
           document.frmMain.estrategia.value = "<%=SolicitacaoAgendamentoEstrategia.STRATEGY_INCLUIR_INICIAR%>";
    	...
    	function configuraCampoDependente(campo){
				...
				if(campo == document.frmMain.registroCombo){
						if(campo.value == "S"){
							//document.frmMain.formaObtencaoBloquetoCombo.disabled = false; <%//AL 07/01/2009 - SISOL 250475%>
		    			...
		    			document.frmMain.aceiteCombo.disabled = false;
							...
							//document.frmMain.formaObtencaoBloquetoCombo.value = '1'; // Informado pelo usuario <%//AL 07/01/2009 - SISOL 250475%>
							//document.frmMain.formaObtencaoBloquetoCombo.disabled = true; <%//AL 07/01/2009 - SISOL 250475%>
