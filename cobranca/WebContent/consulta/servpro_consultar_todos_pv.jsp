...
<%
	 ...
	 *Autor: David L. M. Torneiros - david.torneiros@sao.politec.com.br
	 *Ultima Atualização: 24/05/2007 - v1
	 ************************************************/
%>
...
<%
			ConGerServicosSolicitadosBean filtroBean = (session
			.getAttribute(ServProEstrategia.BEAN_FILTRO) == null ? new ConGerServicosSolicitadosBean()
			: (ConGerServicosSolicitadosBean) session
			.getAttribute(ServProEstrategia.BEAN_FILTRO));

	CedenteCabecaBean cedCabBean = (session
			.getAttribute(ServProEstrategia.BEAN_CABECALHO) == null ? new CedenteCabecaBean()
	...
	if (request.getParameter(ServProEstrategia.PAGINACAO_PAGE) != null
			&& !request.getParameter(ServProEstrategia.PAGINACAO_PAGE)
			.equals("")) {
		paginaAtual = Integer.parseInt((String) request
		.getParameter(ServProEstrategia.PAGINACAO_PAGE));
	} else {
		paginaAtual = Integer.parseInt((String) request
		.getAttribute(ServProEstrategia.PAGINACAO_PAGE));
	}
...
<p:Document>
	<s:FormStrategy name="frmMain" action="SigcbControle"
		...
		<input type="hidden" name="<%=ServProEstrategia.PAGINACAO_PAGE%>"
			value="">
		<%
		//EAM 22/08
		%>
						...
						<table width="100%" border="0" cellspacing="3" cellpadding="0"
							...
							<tr <%= ( ((i+1)%2) == 0 ) ? "class='line0'" : "class='line1'" %>>
								<td width="1%" align="center"><input type="radio"
									name="rdo"
									onclick="javascript: clickRadio('<%=occ.getCodigoCedente()%>')"></td>
								<td width="10%" align="right"><%=occ.getCodigoCedenteFormatado()%></td>
								<td width="80%" align="left">&nbsp;&nbsp;<%=occ.getNomeCedente()%></td>
							...
							<tr>
								<td colspan="6" align="center"><s:ButtonPaginar
									pageNumber="<%=paginacao.getPageNumber()%>"
									numberOfPages="<%=paginacao.getPageCount()%>"
									pageName="<%=ServProEstrategia.PAGE_LISTA_PVVINC%>" /></td>
							</tr>
							<tr>
								<td colspan="6">&nbsp;</td>
							</tr>
