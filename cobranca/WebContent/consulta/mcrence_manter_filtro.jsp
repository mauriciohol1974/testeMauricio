...
*Projeto CAIXA - SIGCB
*Componente: mcrence_manter_filtro.jsp - Java Server Pages
...
<%
  ConGerCedenteRentabilidadeBean filtroBean = (session.getAttribute(MCRenCeEstrategia.BEAN_FILTRO)==null? new ConGerCedenteRentabilidadeBean()
																			:(ConGerCedenteRentabilidadeBean)session.getAttribute(MCRenCeEstrategia.BEAN_FILTRO));
	filtroBean.setCodigoCedente(session.getAttribute(MCRenCeEstrategia.CEDENTE_ATUAL)==null?new Long (0):(Long)session.getAttribute(MCRenCeEstrategia.CEDENTE_ATUAL));
%>

<s:Header/>
  <p:Document>  	
   	<s:FormStrategy name="frmMain" action="SigcbControle" estrategia="<%=MCRenCeEstrategia.STRATEGY_FILTRO%>" fluxo="normal">   	          				
 		<s:Menu/>   		
 		<s:Titulo name="Consultar Rentabilidade do Cedente >> Filtro"/>
